...
# Vendored from sgl-project/sglang python/sglang/srt/function_call/pythonic_detector.py
...
class PythonicDetector(BaseFormatDetector):
    ...
    def detect_and_parse(self, text: str, tools: list[Tool]) -> StreamingParseResult:
        ...
        try:
            ...
            if not (isinstance(parsed, ast.List) and all(isinstance(e, ast.Call) for e in parsed.elts)):
            ...
            for call_index, call in enumerate(parsed.elts):
                ...
                if function_name not in tool_indices:
                    ...
                    if True:  # strict (SGLang default): drop unknown tools
    ...
    def _get_parameter_value(self, val):
        if isinstance(val, ast.Constant):
            ...
            return {k.value: self._get_parameter_value(v) for k, v in zip(val.keys, val.values)}
