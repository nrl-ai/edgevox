...
# Vendored from sgl-project/sglang python/sglang/srt/function_call/mistral_detector.py
# (Apache-2.0). Import paths rewritten for EdgeVox; detection logic unchanged.
import json
import logging
...
from edgevox.llm.tool_parsers.utils import _is_complete_json
...
class MistralDetector(BaseFormatDetector):
    ...
    def detect_and_parse(self, text: str, tools: list[Tool]) -> StreamingParseResult:
        ...
        if self.bot_token in tool_part:
            ...
            try:
                ...
                logger.warning(f"Failed to parse JSON part: {json_array_str}, JSON parse error: {e!s}")
            json_pos = tool_part.find(json_array_str) if json_array_str else -1
            trailing_text = tool_part[json_pos + len(json_array_str) :].strip() if json_pos != -1 else ""
            combined_normal = (normal_text + " " + trailing_text).strip() if trailing_text else normal_text
        ...
        while remaining:
            ...
            new_calls = self.parse_base_json({"name": func_name, "arguments": args_obj}, tools)
        ...
        combined_normal = (normal_text + " " + remaining).strip() if remaining else normal_text
    ...
    def _try_parse_compact_args_format(self, text: str) -> tuple[str, Any, int] | None:
        """
    ...
    def _extract_json_value(self, text: str, json_start: int) -> tuple[str | None, int]:
        """
